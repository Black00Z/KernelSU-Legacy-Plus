...
fn validate_module_id_legacyplus(id: &str) -> Result<()> {
...
pub fn run_action(id: &str) -> Result<()> {
    validate_module_id_legacyplus(id)?;
    let action_script = Path::new(defs::MODULE_DIR).join(id).join("action.sh");
    ensure!(action_script.is_file(), "Module action.sh not found: {id}");
...
fn exec_script<T: AsRef<Path>>(path: T, wait: bool) -> Result<()> {
    ...
    // Modern KernelSU exposes the owning module id to module scripts.
    // Backport that small environment contract as well so newer action/WebUI
    // helper scripts do not have to special-case the legacy 1.0.1 userspace.
    let module_id = path
        .as_ref()
        ...
        .filter(|id| validate_module_id_legacyplus(id).is_ok())
...
fn _list_modules(path: &str) -> Vec<HashMap<String, String>> {
    ...
    for entry in dir.flatten() {
        ...
        let action = path.join("action.sh").is_file();
